...
import { buildRequestFileObject, getMissingParams, isEmptyObject, isFileParam } from 'ibm-cloud-sdk-core';
...
const allowedAxiosConfig = ['httpsOverHttp', 'transformRequest', 'transformResponse', 'paramsSerializer', 'paramsSerializer', 'timeout', 'withCredentials', 'adapter', 'responseType', 'responseEncoding', 'xsrfCookieName', 'xsrfHeaderName', 'onUploadProgress', 'onDownloadProgress', 'maxContentLength', 'validateStatus', 'maxRedirects', 'socketPath', 'httpAgent', 'httpsAgent', 'proxy', 'cancelToken'];
...
export class RequestWrapper {
  ...
  constructor(axiosOptions?) {
    ...
    // temp - allow option to specify tunnel for https through http proxy
    // these options should be configurable by the user, but this method could be a nice convenience
    if (axiosOptions.httpsOverHttp === true) {
      // dont actually pass this into axios
      delete axiosConfig.httpsOverHttp;

      const agent = tunnel.httpsOverHttp({
          // users need to specify the proxy as if it were going straight to axios
